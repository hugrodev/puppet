...
export class Solver {
  ...
  solve() {
    const result: { values: number[] | null; loss: number; } = this.solveNarrow(this.solveWide());
    return {
  ...
  solveWide() {
    ...
    let best: { values: number[] | null; loss: number; } = { values: [], loss: Infinity };
